...
int is_valid_lba(char *str) { // LBA 유효성 검사 함수
...
int is_valid_value(char *str) {
    if (str == NULL) return 0;
    if (strncmp(str, "0x", 2) != 0) return 0; // 0x로 시작하는지, 10자리(0x포함)인지 확인

    if (strlen(str) != 10) return 0; // 길이가 10인지 확인

    for (int i = 2; i < 10; i++) {  // 16진수 문자 검사
...
int main() {
    ...
    while (1) {
        printf(">> ");
        if (fgets(input, MAX_CMD_SIZE, stdin) == NULL) break;
